...
function renderBtns (h, _vm) {
  const { $slots, $xegrid, $xetable, buttons = [] } = _vm
  if ($slots.buttons) {
    return $slots.buttons.call(_vm, { $grid: $xegrid, $table: $xetable }, h)
  }
  return buttons.map(item => {
    ...
    return h(resolveComponent('vxe-button'), {
      onClick: evnt => _vm.btnEvent(evnt, item),
      icon,
      type,
      status,
      disabled,
      loading,
      slots: dropdowns && dropdowns.length ? {
        default: () => UtilTools.getFuncText(name),
        dropdowns: () => dropdowns.map(child => {
          return child.visible === false ? null : h(resolveComponent('vxe-button'), {
            onClick: evnt => _vm.btnEvent(evnt, child),
            icon: child.icon,
            type: child.type,
...
function renderRightTools (h, _vm) {
  const { $slots, $xegrid, $xetable } = _vm
  if ($slots.tools) {
    return $slots.tools.call(_vm, { $grid: $xegrid, $table: $xetable }, h)
  }
  return []
}
...
function renderCustoms (h, _vm) {
  ...
  if (customOpts.trigger === 'manual') {
    ...
    // 弹窗
    customBtnOns.onClick = _vm.customPanelEvent
  } else if (customOpts.trigger === 'hover') {
    // hover 触发
    customBtnOns.onMouseenter = _vm.handleMouseenterSettingEvent
    customBtnOns.onMouseleave = _vm.handleMouseleaveSettingEvent
    ...
    // 点击触发
    customBtnOns.onClick = _vm.handleClickSettingEvent
  }
  ...
  return h('div', {
    ...
    h('div', {
      ...
      customOpts.isFooter === false ? null : h('div', {
        ...
        h('button', {
          class: 'btn--confirm',
          onClick: _vm.confirmCustomEvent,
        }, GlobalConfig.i18n('vxe.toolbar.customConfirm')),
        h('button', {
          class: 'btn--reset',
          onClick: _vm.resetCustomEvent,
  ...
  render (h) {
    ...
    return h('div', {
      ...
      h('div', {
        ...
        this.import ? h(resolveComponent('vxe-button'), {
          circle: true,
          icon: importOpts.icon || GlobalConfig.icon.TOOLBAR_TOOLS_IMPORT,
          title: GlobalConfig.i18n('vxe.toolbar.import'),
          onClick: this.importEvent
        }) : null,
        this.export ? h(resolveComponent('vxe-button'), {
          circle: true,
        ...
        this.print ? h(resolveComponent('vxe-button'), {
          circle: true,
        ...
        zoom && $xegrid ? h(resolveComponent('vxe-button'), {
          ...
          onClick: $xegrid.triggerZoomEvent,
        }) : null,
