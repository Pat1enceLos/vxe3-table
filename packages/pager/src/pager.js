...
export default {
  ...
  render () {
    const { $slots, $xegrid, vSize, align } = this
    const childNodes = []
    if ($slots.left) {
      childNodes.push(
        h('span', {
          class: 'vxe-pager--left-wrapper'
        }, $slots.left.call(this, { $grid: $xegrid }))
      )
    }
    ...
    if ($slots.right) {
      childNodes.push(
        ...
        }, $slots.right.call(this, { $grid: $xegrid }))
      )
    }
    return h('div', {
      class: ['vxe-pager', {
  ...
  methods: {
    ...
    renderJump (h, isFull) {
      return h('span', {
        ...
        h('input', {
          ...
          type: 'text',
          autocomplete: 'off',
          onKeydown: this.jumpKeydownEvent,
          onBlur: this.triggerJumpEvent,
        }),
        isFull ? h('span', {
    ...
    renderPageBtn (h, showJump) {
      ...
      if (showJump && isGt) {
        nums.push(
          ...
          h('li', {
            class: 'vxe-pager--num-btn',
            onClick: () => this.jumpPage(pageCount)
          }, pageCount)
        )
      }
      return nums
    ...
    jumpPage (currentPage) {
      if (currentPage !== this.currentPage) {
        ...
        this.$emit('pageChange', { type: 'current', pageSize: this.pageSize, currentPage, $event: { type: 'current' } })
    ...
    changePageSize (pageSize) {
      if (pageSize !== this.pageSize) {
        ...
        this.$emit('pageChange', { type: 'size', pageSize, currentPage: Math.min(this.currentPage, this.getPageCount(this.total, pageSize)), $event: { type: 'size' } })
        const key = this.id || window.location.href
