...
export default {
  ...
  render () {
    const { $slots, $attrs, type, isFormBtn, btnStatus, btnType, vSize, name, disabled, loading, showPanel, animatVisible, panelPlacement } = this
    return $slots.dropdowns ? h('div', {
      ...
      h('button', {
        ...
        ...XEUtils.objectMap($attrs, (cb, type) => evnt => this.$emit(type, { $event: evnt }))
      ...
      h('div', {
        ...
        h('div', {
          ...
          onClick: this.clickDropdownEvent,
          onMouseenter: this.mouseenterEvent,
          onMouseleave: this.mouseleaveEvent
        }, $slots.dropdowns.call(this))
      ])
    ]) : h('button', {
      ref: 'btn',
      class: ['vxe-button', `type--${btnType}`, {
  ...
  methods: {
    renderContent (h) {
      const { $slots, content, icon, loading } = this
      const contents = []
      if (loading) {
        contents.push(
          h('i', {
