...
import { throttle } from 'xe-utils/methods'
...
const Methods = {
  ...
  calcTableWidth () {
    this.$nextTick(() => {
      ...
      const myObserver = new ResizeObserver((entries) => {
        entries.forEach(entry => {
          this.$nextTick(() => {
            ...
            const ref = `${key}ScrollWrapper`
            if (this.$refs[ref] && this.$refs[ref].clientWidth) {
              const clientWidth = parseInt(this.$refs[ref].clientWidth, 10)
              const sumExceptLast = XEUtils.sum(this.dropRight(columns.map((i) => i.width)))
              const columnsLength = columns.length
              storedData[key][columnsLength - 1] = clientWidth - sumExceptLast > 40 ? clientWidth - sumExceptLast : 40
              columns[columnsLength - 1].width = clientWidth - sumExceptLast > 40 ? clientWidth - sumExceptLast : 40
      ...
      myObserver.observe(this.$refs[ref])
      const tmp = localStorage.getItem('columnWidth')
      const storedData = tmp ? JSON.parse(tmp) : {}
      const clientWidth = parseInt(this.$refs[ref].clientWidth, 10)
      if (!storedData || !storedData[key]) {
        ...
        storedData[key] = columns.map((i) => i.width)
      }
      const sumExceptLast = XEUtils.sum(this.dropRight(storedData[key]))
      const columnsLength = columns.length
      columns.forEach((i, index) => {
        if (index === columnsLength - 1) { // 空白列
      ...
      this.recordWidth = XEUtils.sum(storedData[key])
      localStorage.setItem('columnWidth', JSON.stringify(storedData))
      setTimeout(() => {
        this.refreshColumn()
        this.loadColumn(columns)
  ...
  hideDropdown () {
    ...
    const resizeObserver = new ResizeObserver(this.hideDropdownOperation)
    const mainRef = `${key}-table`
    const header = this.$refs[mainRef].querySelector('.vxe-table--header')
    if (header) {
      // 拖动表头关闭打开的dropdown
  ...
  hideDropdownOperation () {
    ...
    if (triggerElm &&
      triggerElm.classList.contains('ant-dropdown-trigger') &&
      triggerElm.classList.contains('ant-dropdown-open')) {
