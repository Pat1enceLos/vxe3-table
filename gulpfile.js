...
gulp.task('build_modules', () => gulp.src('packages/**/*.js')
  .pipe(babel({
    presets: ['@babel/env'],
  }))
  .pipe(gulp.dest('lib'))
...
gulp.task('build_i18n', () => Promise.all(languages.map((code) => {
...
gulp.task('copy_ts', () => gulp.src('packages/**/*.d.ts')
  .pipe(gulp.dest('lib')));

gulp.task('lib_rename', () => Promise.all([
...
gulp.task('build_style', gulp.series('build_modules', 'build_i18n', 'copy_ts', () => Promise.all(components.map((name) => {
  Promise.all([
...
gulp.task('build_clean', gulp.series('build_style', 'lib_rename', () => gulp.src([
  'lib/demo.html',
])
  .pipe(clean())));
